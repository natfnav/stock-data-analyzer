...
{
    /// <summary>
    /// Class representing a Valley pattern recognizer; a derivation of a Recognizer tailored for
    /// identifying Valley candlesticks
    /// </summary>
    class Recognizer_Valley : Recognizer
    {
        /// <summary>
        /// Constructor that creates a Recognizer_Valley object and initializes its respective
        ...
        /// <returns>
        /// A boolean value for the current candlestick; returns true if a Valley pattern is detected
        /// and false otherwise
        /// </returns>
        public override bool Recognize(List<SmartCandlestick> scsList, int currIndex)
        {
            ...
            if (currCS.Patterns[PatternName] == true)
                ...
                r = true;
            // Checks if next candlestick is already involved in a Peak pattern
            if (nextCS.Patterns[PatternName] == true)
                // Maintain true value for Peak pattern entry
                nextCS.Patterns[PatternName] = true;

            // Check for Valley pattern
            if ((prevCS.Low > currCS.Low) && (currCS.Low < nextCS.Low))
            {
